package com.fengdi.baseproject.mvp.ui.widget;
...
import com.jess.arms.utils.ArmsDeviceUtils;
...
public class MyToolBar extends Toolbar implements View.OnClickListener {
    ...
    public MyToolBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        //是否需要考虑状态栏
        boolean isNeedStatusbarPadding = typedArray.getBoolean(R.styleable.MyToolBar_isNeedStatusbarPadding, false);

        if (isNeedStatusbarPadding) {
            //设置标题栏padding
            ViewGroup.LayoutParams linTitleBarLayoutParams = linTitleBar.getLayoutParams();
            linTitleBarLayoutParams.height = linTitleBarLayoutParams.height + ArmsDeviceUtils.getStatusBarHeight(context);
            linTitleBar.setLayoutParams(linTitleBarLayoutParams);
            linTitleBar.setPadding(linTitleBar.getPaddingLeft(), linTitleBar.getPaddingTop() + ArmsDeviceUtils.getStatusBarHeight(context), linTitleBar.getPaddingRight(), linTitleBar.getPaddingBottom());
        }
